...
static void strcmd_add(Storage *str, StorageCommandType type, const char* data,
                       size_t index, size_t length, StorageSaveMode savemode)
{
    if (savemode == STR_NONE) {
        return;
    }
...
Storage* storage_alloc(size_t size)
{
    ...
    s->front_size =
        size; //because the front buffer doesnt like being zero and will cause a weird initial state that has to be reset by intserting a newline
...
void storage_move_gap(Storage *str, size_t pos)
{
    ...
    long long bytesToMove = str->front_size -
                            pos; //encodes both amount and direction to shift bytes
                   ...
                   StorageSaveMode savemode)
{
    ...
    str->gap_size += count;
    strcmd_add(str, STORAGE_REMOVE, (str->buffer + str->front_size), pos, count,
               savemode);
    storage_move_gap(str, pos);
