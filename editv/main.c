...
#undef DEBUG //Comment out enable debug output

bool do_draw =
    true; //lock this when doing callbacks that modify/replace the storage or they may be a chance of memory corruption
...
bool cache_lines =
    false; //CURRENTLY BROKEN AND CAUSES MAJOR GLITCHES, DO NOT ENABLE
...
void Update_Line_Wrap_Count()
{
    ...
    while (index <= STR_END(str)) {
        ...
        if (wrap_lines) {
            ...
            if (TTF_MeasureString(font, buf, l, (int)(info.w - info.xorigin - info.cw),
                                  NULL, &measured)) {
                if (measured < l) {
                    //curLine--;
                    ++linedata.wrap_count;
...
int ParseArgs(int argc, char* argv[])
{
    ...
    for (size_t i = 1; i < argc; i++) { //loop over args looking for flags
        ...
        if (argv[i][0] == '-') {
            if (len < 2)
                goto
                invalid_args;//make sure theres enough space to not cause overflow
...
void SaveTo(const char* path)
{
    ...
    SDL_WriteIO(stream, str->buffer + str->front_size + str->gap_size,
                str->buffer_size - str->front_size - str->gap_size * sizeof(char));
    SDL_CloseIO(stream);
...
SDL_Texture* CacheLine(line_t line, char* buf, SDL_Color color)
{
    ...
    if (text) {
        ...
        cached_lines[line.number - line_start] = (cached_line) {
            line, texture, 1
        };
...
render_info GetRenderInfo(TTF_Font *font)
{
    ...
    info.text_color = (SDL_Color) {
        cfg->text_color.r, cfg->text_color.g, cfg->text_color.b, SDL_ALPHA_OPAQUE
    };
    return info;
}
...
int GetCursorPos(size_t* cursor_x, size_t* cursor_y, line_t lines[],
                 size_t line_count)
{
    //get cursor pos
            ...
            if ((lines[line_count - 1].index + lines[line_count - 1].length) < cursor_pos) {
                ...
                *cursor_x = 0;
                ...
                for (size_t i = 0; i < line_count; i++) {
                    if (cursor_pos >= lines[i].index &&
                            cursor_pos <= (lines[i].index + lines[i].length)) {
...
void WrapCursor(size_t* cursor_x, size_t* cursor_y, line_t lines[],
                size_t line_count, size_t max_y)
{
    int new_y = (int) * cursor_y + y_offset;
    new_y = SDL_max(new_y, 0);//keep this above zero to avoid weird wrapping errors

    if (line_count > max_y &&
            new_y >= line_count + line_start) { //off bottom of page
        ...
        if (line_count > 2) {
            int in = (int)lines[line_count - 1].index + (int)lines[line_count -
                     1].length; // points to the end of the last visible line
            in++;//skip over newline
            size_t i;

            for (i = 0; i < *cursor_x;
                    i++) { //ensure that we dont accidentally skip multiple lines
            ...
            index_offset =
                lines[newline].index; //we have already cached the line index of the first line
            line_start += newline;
            cursor_pos = in + i;
...
void DrawMenu(render_info* info, size_t cursor_x, size_t cursor_y)
{
    ...
    if (func_mode) {
        if (shift_down) {
            ...
            for (size_t i = 0; i < sizeof(alt_menu_options) / sizeof(alt_menu_options[0]);
                    i++) {
                stride += SDL_snprintf(buf + stride, 256 - stride, "   %s",
                                       alt_menu_options[i]);
            }
        } else {
        ...
        if (tt) {
            ...
            if (ttxt) {
                ...
                if (TTF_GetTextSize(ttxt, &tw, &th)) {
                    ...
                    SDL_SetRenderDrawColor(renderer, info->text_color.r, info->text_color.g,
                                           info->text_color.b, info->text_color.a);
...
void DrawLineNumbers(render_info* info, line_t lines[], size_t line_count,
                     size_t cursor_y)
{
    ...
    for (size_t i = 0; i < line_count; i++) {
        if (lines[i].number !=
                last) { //dont draw multiple line numbers for wrapped lines. perhaps make this an option in the future
            ...
            RenderTextAt(info->cw, (float)(info->yorigin + info->line_gap * i), buf,
                         lineColor);
        }
    }
}
...
//draws lines
void DrawLines(render_info *info, line_t* lines, size_t max_lines,
               size_t* line_count)
{
    *line_count = 0;
    int x = info->xorigin;
    int y = info->yorigin;
    size_t curLine =
        line_start; //line offset into array, we need this for proper cursor placement. cached so that we dont have to loop the entire array every time we render
...
#define buflen 1024
    ...
    while (index <= STR_END(str)) {
        ...
        if (!cache_lines || (!cached_lines[curLine - line_start].filled ||
                             cached_lines[curLine - line_start].line.number != curLine)) {
...
void Draw()
{
    ...
    line_t* lines = SDL_malloc(max_lines * sizeof(
                                   line_t)); //need an extra line for newlines
    //main drawing loop
    DrawLines(&info, lines, max_lines, &line_count);
    size_t cursor_x, cursor_y;

    if (!GetCursorPos(&cursor_x, &cursor_y, lines,
                      line_count)) { //set defaults if it cant find the position, so we dont try jump to uninitialised memory
        cursor_x = 0;
        cursor_y = line_start;
    }
    ...
    if (cache_lines)
        cached_lines[cursor_y - line_start].filled =
            0; //dont cache the line that the cursor is on
...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char* argv[])
{
    ...
    /* Create the window */
    if (!SDL_CreateWindowAndRenderer(appname, 800, 600, SDL_WINDOW_RESIZABLE,
                                     &window, &renderer)) {
...
SDL_AppResult SDL_AppIterate(void* appstate)
{
    if (do_draw) { //else just freeze whatever is currently on screen
        SDL_SetRenderDrawColor(renderer, cfg->background_color.r,
                               cfg->background_color.g, cfg->background_color.b, cfg->background_color.a);
