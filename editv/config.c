...
void print_arg(FILE *f, cfg_type type, const char* name, void* value)
{
    ...
    switch (type) {
        ...
        case edv_color_cfg:
            SDL_snprintf(buf, 256, "%.2X%.2X%.2X%.2X", ((edv_color*)value)->r & 0xff,
                         ((edv_color*)value)->g & 0xff, ((edv_color*)value)->b & 0xff,
                         ((edv_color*)value)->a & 0xff);
            break;
...
edv_config* load_config(void)
{
    ...
    memcpy(cfg->default_font, cfg_defaults.default_font,
           strlen(cfg_defaults.default_font) +1);
    ...
    if (SDL_GetPathInfo(config_path, NULL)) {
        ...
        while (fgets(buf, 1024, f)) {
            ...
            if (rvalue == NULL) {
                continue;    // only happens when its 'name=' and the rvalue is completely blank
            }

            //remove leading and trailing spaces
            while (*rvalue == ' ') {
                rvalue++;    //skip over spaces
            }
